...
class ModeControl:
    ...
    def toggle_relay(self, pin, state_var):
        if state_var.get() == "Momentary":
            ...
            self.app.root.after(100, lambda p=pin: GPIO.output(p, GPIO.LOW))
        else:
            GPIO.output(pin, not GPIO.input(pin))
    ...
    def toggle_stepper_direction(self):
        if self.stepper_enabled:
            ...
            GPIO.output(self.app.dir_pin, self.stepper_direction)
            self.update_status_label()

    def toggle_stepper_motor(self):
        if self.stepper_enabled:
            GPIO.output(self.app.enable_pin, GPIO.HIGH)
            self.stepper_enabled = False
            ...
            GPIO.output(self.app.enable_pin, GPIO.LOW)
    ...
    # TODO: check if the function call is correct
    def move_stepper_motor(self):
        if self.stepper_enabled:
            GPIO.output(self.app.step_pin, GPIO.HIGH)
            self.app.root.after(step_rate, self.move_stepper_motor)
